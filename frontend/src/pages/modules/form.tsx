...
import {
  getApiV1ModulesById,
  type ModuleReq,
  postApiV1CoursesByIdModulesMutation,
  putApiV1ModulesByIdMutation,
...
export function ModuleForm(props: FormProps) {
  const { mutate: create } = useMutation(postApiV1CoursesByIdModulesMutation())
  const { mutate: update } = useMutation(putApiV1ModulesByIdMutation())
  ...
  const getDefaultValues = useEffectEvent(() => {
    if (props.mode === "create") {
      ...
      getApiV1ModulesById({ path: { id: props.id } })
        .then(res => {
          if (!res.data) return
